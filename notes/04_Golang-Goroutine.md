...
- 上下文
	...
	- 上下文切换指的是内核(操作系统的核心)在CPU上对进程或者线程进行切换
		- 上下文切换过程中的信息被保存在进程控制块(PCB-Process Control Block)中
			- PCB又被称作切换帧(SwitchFrame)
			- 上下文切换的信息会一直被保存在CPU的内存中，直到被再次使用
	- [Linux进程上下文概念](https://blog.csdn.net/wuxing26jiayou/article/details/104883455)
...
- 锁
	- 锁的本质，就是⼀种资源，是由操作系统维护的⼀种专⻔⽤于同步的资源
	- 用于解决隔离性的一种机制
	...
	- Go语言中的锁
		...
		- 读写锁
	- [Golang 锁实现原理](https://blog.csdn.net/zhw21w/article/details/129488087)
...
### 3. 原子操作
 - 原子操作
	- 指不会被线程调度机制打断的操作，即针对某个值的原子操作过程中CPU绝对不会再去进行其它操作
	- 不能中断，不能被终端
...
- Golang中的原子操作
	- 非入侵式的，由标准库`sync/atmoic`包提供了原子操作的能力，直接由底层CPU硬件支持，因而比基于操作系统API的锁方式效率更高
	- 数据类型
		- `int32`
		- `int64`
		- `uint32`
		...
		- `uintptr`
		- `unsafe.Pointer`
	- 种类，`sync/atomic`包中提供了五类原子操作函数，其操作对象为整数型或整数指针
		- `Add*`: 增加/减少
		- `Load*`: 载入
		- `Store*`: 存储
		- `Swap*`: 更新/交换
		- `CompareAndSwap*`
			- CAS函数(Compare And Swap)，比较并交换，是sync锁操作的基础，`sync.mutex`是基于CAS方法来判断是否获取了锁
			- 比较第一个参数引用值是否与第二个参数值相同，若相同则将第一个参数值更新为第三个参数
			- 比较并交换 `cas`操作实际上是带条件的交换 `swap`操作，只有符合条件才交换
	- Golang在 `sync`包中已经提供了锁，为什么还需要使用 `atomic`原子操作
		- 加锁的代价比较耗时，需要上下文切换
		- 原子操作只针对基本数据类型，可以使用原子操作保证线程安全
		- 原子操作在用户态可以完成，性能比互斥锁要高
		- 针对特定需求原子操作步骤简单，无需加锁解锁步骤
	- [Go atomic](https://www.jianshu.com/p/a6a86d8e3258)
